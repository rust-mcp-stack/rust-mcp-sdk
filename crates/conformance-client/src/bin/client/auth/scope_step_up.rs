...
use rust_mcp_sdk::schema::{CallToolRequestParams, ListToolsResult, RequestMetaObject};
...
pub async fn handle(
    ...
    // SEP-2352: re-run discovery before re-authenticating. A 401 can signal
    // that the protected resource's `authorization_servers` changed (AS
    // migration); the client MUST discover the new AS and re-register with
    // it rather than reuse credentials issued by the previous one. Fall back
    // to the original discovery output if re-discovery fails.
    let fresh_discovery = super::discovery::resolve(http, server_url, context).await;
    let discovery = fresh_discovery.as_ref().unwrap_or(discovery);
